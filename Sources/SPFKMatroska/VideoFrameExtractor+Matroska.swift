    ...
    static func cgImages(
        ...
        for timestamp in timestamps.sorted() {
            do {
                ...
                // No index: only the frames already ahead of the walk are reachable, so the rest of
                // the strip is left empty rather than filled with the same picture repeatedly.
                if images.isEmpty == false { break }
            }

            guard let frame = try decoder.nextImage(), let image = frame.cgImage else { continue }

            images[timestamp] = maximumSize.flatMap { image.scaledPreservingAspect(to: $0) } ?? image
        }
