...
+ (nullable MKVSegmentDescription *)readSegmentDescriptionAtURL:(NSURL *)url
    ...
    if (mkvparser::Segment::CreateInstance(&reader, position, rawSegment) != 0 ||
        rawSegment == nullptr) {
        if (error != nullptr) {
    ...
    NSMutableArray<MKVTrackDescription *> *descriptions =
        [NSMutableArray arrayWithCapacity:trackCount];
